...
/// 列表选中行底色 —— 原版 Log 选中行实测灰调 `#33353B`
///（蓝色 `#2A4371` 只出现在菜单高亮/主操作，不用于 Log 行选中）。
...
/// Log 提交列表专属底色 —— 原版实测蓝调 `#1D2336`（同屏编辑器/侧栏为
/// `#191A1C`，仅 Log 列表底色偏蓝）。
...
/// HEAD 提交的橙色圆环节点色（原版实测：环 + 中心点同色，约 `#BE8E48`）。
pub fn head_dot_color() -> Hsla {
    hsla(0.083, 0.52, 0.51, 1.0)
}
...
const LANES: [(f32, f32, f32); MAX_COLORS] = [
    (0.167, 0.43, 0.42), // 黄绿 #99993D（原版实测首泳道）
    (0.585, 0.61, 0.38), // 蓝 #185FA0（原版实测第二泳道 / merge 圈）
    (0.13, 0.90, 0.62),  // 琥珀黄
